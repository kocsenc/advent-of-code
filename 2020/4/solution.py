...
hcl_re = re.compile(r'hcl:#([0-9a-f]{6})') # 6 digit hex
ecl_re = re.compile(r'ecl:(amb|blu|brn|gry|grn|hzl|oth)')
pid_re = re.compile(r'pid:(\d{9}\s)')
...
def main(filename='input.txt'):
    ...
    def is_valid_passport(passport_string):
        ...
        return all([re.search(passport_string) for re in required_fields_re])

    valid_passports = list(filter(is_valid_passport, passport_entries))
    print('Valid Passports: {}/{}'.format(len(valid_passports), len(passport_entries)))
